...
export const MultimodalPreview: React.FC<MultimodalPreviewProps> = ({
    ...
    return (
      <div className={cn("relative inline-block", className)}>
        ...
        {removable && (
          <button
            type="button"
            className="absolute top-1 right-1 z-10 rounded-full bg-muted-foreground text-white hover:bg-foreground"
            onClick={onRemove}
  ...
  if (block.type === "file" && block.mimeType === "application/pdf") {
    ...
    return (
      <div
        className={cn(
          "relative flex items-start gap-2 rounded-md border bg-secondary px-3 py-2",
          className,
        )}
      >
        ...
        {removable && (
          <button
            type="button"
            className="ml-2 self-start rounded-full bg-muted p-1 text-teal-700 hover:bg-accent"
            onClick={onRemove}
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 rounded-md border bg-secondary px-3 py-2 text-muted-foreground",
        className,
      )}
    >
